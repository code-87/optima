...
class ModelCatalogOCFilter extends Model {
  ...
  private function setCopyFilterUrlAlias($data) {
    ...
    // Rename Duplicates
    $query = $this->db->query("SELECT oov.value_id FROM " . DB_PREFIX . "ocfilter_option_value oov LEFT JOIN " . DB_PREFIX . "ocfilter_option oo ON (oov.option_id = oo.option_id) LEFT JOIN " . DB_PREFIX . "url_alias ua ON (oov.keyword = ua.keyword) WHERE oo.status = '1' AND oov.keyword != '' AND ua.keyword IS NOT NULL");
