			...
			foreach($socnets as $i=>$socnet)
			{
				...
				if( $this->getConfigPostValue('socnetauth2_'.$socnet['key'].'_agent') != 'loginza' )
				{
					if( $socnetauth2_shop_folder && $this->getConfigPostValue( 'socnetauth2_shop_folder_in_url' ) )
						$link = '{shop_folder}/socnetauth2/'.$socnet['key'].'.php?first=1';
					else
				...
				if( $socnetauth2_shop_folder && $this->getConfigPostValue('socnetauth2_shop_folder_in_img') )
					$img = '{shop_folder}'.$img;
				...
				$template = str_replace("{title_".$index."}", "{title_".$socnet['short']."}", $template);
			}
