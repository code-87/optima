...
class ControllerProductProduct extends Controller {
	...
	public function index() {
        ...
        //OC-HELP.com Viewed Products START
        $this->load->model('catalog/ochelp_viewed');

        if($this->config->get('ochelp_viewed_storage_limit')){
            $storage_limit = $this->config->get('ochelp_viewed_storage_limit');
        }else{
            $storage_limit = '30';
        }

        if($this->config->get('ochelp_viewed_product_limit')){
            $product_limit = $this->config->get('ochelp_viewed_product_limit');
        }else{
            $product_limit = '50';
        }

        if(!isset($this->request->cookie['viewed_product'])){
            $this->model_catalog_ochelp_viewed->deleteOldViewedProduct($storage_limit);

            $session_token = token('8');
             setcookie('viewed_product', $session_token, time() + 30 * 60 * 24 * (int)$storage_limit, '/', $this->request->server['HTTP_HOST']);
        }else{
            $session_token = $this->request->cookie['viewed_product'];
        }
        
        $this->model_catalog_ochelp_viewed->addViewedProduct($session_token, $this->request->get['product_id'], $product_limit);
        //OC-HELP.com Viewed Products END
...
$data['price_one'] = $this->currency->format($this->tax->calculate($product_info['price'] / $product_info['minimum'],  $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
