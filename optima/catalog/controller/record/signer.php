		...
		if ((isset($settings['signer_answer']) && isset($settings['signer'])) && ($settings['signer'] || $settings['signer_answer'] || $notify_status || $settings['comments_email'] != '')) {
			...
			if ($this->registry->get('admin_work')) {
				...
				if (defined(DIR_CATALOG) || @DIR_CATALOG != '') {
					$dir_seo_type = DIR_CATALOG;
				} else {
					$dir_seo_type = DIR_APPLICATION;
				}

				if ($mark_id == 'record_id') {
					require_once($dir_seo_type . 'controller/common/seoblog.php');
					...
					if (SC_VERSION > 21) {
						$seo_pro_file = $dir_seo_type . 'controller/startup/' . $seo_type . '.php';
   						$classSeo = 'ControllerStartup' . str_replace('_', '', $seo_type);
					} else {
						$seo_pro_file = $dir_seo_type . 'controller/common/' . $seo_type . '.php';
   						$classSeo = 'ControllerCommon' . str_replace('_', '', $seo_type);
   					}
                    if (file_exists($seo_pro_file)) {
						if (function_exists('modification')) {
                        	require_once(modification($seo_pro_file));
						} else {
							require_once($seo_pro_file);
						}
						if (!class_exists($classSeo, false)) {
			...
			if (is_string($this->config->get('config_logo')) && $this->config->get('config_logo') && file_exists(DIR_IMAGE . $this->config->get('config_logo'))) {
