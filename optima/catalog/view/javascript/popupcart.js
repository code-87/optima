...
$(document).ready(function(){
	...
	if (!localStorage.getItem('p_button')) {
		$('.button-cart,.cart .button, .btn-group .btn, .btn-group .btn-primary, .button-group button').each(function() {
	...
	//	p_array();
		...
		if($('input[name=\'click_on_cart\']').val() == 1) {
			if($('.call-cart > .heading a').length) {
				$('.call-cart > .heading a').off('click');
			}
			$('.call-cart').bind('hover', function (e) { 
				$('.call-cart .content').remove();
			});
			$('.call-cart').bind('click', function (e) { 
				e.preventDefault();
				$('.call-cart .content').remove();
				$('.call-cart .dropdown-menu').remove();

				$('.modal-cart').load('index.php?route=extension/module/popupcart .modal-cart > *', function() {
					$('.modal-cart').popup('show');
				});
			});
		}
	});
...
var cart = {
	...
	$.ajax({
		...
		success: function(json) {
			...
			if (json['success']) {
				$('.call-cart').load('index.php?route=common/cart/info .call-cart > *');
					$('.modal-cart').load('index.php?route=extension/module/popupcart .modal-cart > *', function() {
	...
	'remove': function(key) {
		$.ajax({
			...
			dataType: 'json',
			beforeSend: function() {
				$('.call-cart > span').button('loading');
			},
			complete: function() {
				$('.call-cart > span').button('reset');
			},			
			success: function(json) {
				setTimeout(function () {
					$('.call-cart > span').html('<span id="cart-total"><i class="fa fa-shopping-cart"></i> ' + json['total'] + '</span>');
				}, 100);
				...
				if (getURLVar('route') == 'checkout/cart' || getURLVar('route') == 'checkout/checkout') {
					...
					$('.call-cart > ul').load('index.php?route=common/cart/info ul li');
				}
			}
		});
	}
...
//	if($('.'+product_id).attr('class') == 'button-cart') {
