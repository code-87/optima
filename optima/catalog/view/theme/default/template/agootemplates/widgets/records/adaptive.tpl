   ...
   <div class="seocmspro_content blog-content">
			...
			<style>
			 .record_columns .column_width_<?php echo $cmswidget; ?> {
			 	width: <?php if ((isset ($settings_widget['block_records_width']) && $settings_widget['block_records_width']!='') && (!isset ($settings_widget['slider']) || !$settings_widget['slider'])) {
...
echo 'colorbox_modal';
                   ...
                   <?php if ((isset ($settings_widget['images_view']) && $settings_widget['images_view']) || (isset($settings_widget['avatar_status']) && $settings_widget['avatar_status'])) { ?>
					<div class="image blog-image sc-widget-image-block <?php if (isset($settings_widget['title_status']) && $settings_widget['title_status'] && (isset($settings_widget['title_position']) && $settings_widget['title_position']=='after')) { ?>  <?php } ?>">
					<?php if ($record['thumb'] && isset($settings_widget['avatar_status']) && $settings_widget['avatar_status']) { ?>
						<div class="image sc-widget-image <?php if (isset($settings_widget['title_status']) && $settings_widget['title_status'] && (isset($settings_widget['images_position']) && $settings_widget['images_position']=='after')) { ?> blog-image <?php } ?>">
...
echo 'colorbox_modal';
					...
					<?php if (isset ($settings_widget['images_view']) && $settings_widget['images_view'] ) { $number_hide = 1; ?>
						...
						<div class="image blog-image-thumb <?php if (isset($settings_widget['images_number']) && isset($settings_widget['images_number_hide']) && $settings_widget['images_number_hide'] && $settings_widget['images_number']!='' && $number_hide > $settings_widget['images_number']  ) { ?> asc_displaynone  <?php } ?>">
	...
	<?php } ?>
			...
			<?php
				...
				foreach ($records as $num => $record) {
					if (isset($record['category']['status']) && $record['category']['status']) {
						$category[$record['blog_name']] = $record['blog_href'];
					}
				}
			?>
			<div class="marginbottom5 textalignright">
				<?php if (isset($category) && !empty($category)) { foreach ($category as $name => $href) { ?>
...
$('#record_columns-<?php echo $cmswidget; ?>').owlCarousel({
	items: <?php if (isset($settings_widget['block_records_width']) && $settings_widget['block_records_width'] != '') {
			 		echo round(100 / trim(str_replace('%', '', $settings_widget['block_records_width'])));
			 	} else {
			 		echo count($records);
			 	}
			 	?>,
