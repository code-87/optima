...
<?php echo $box_begin; ?>
	  <div class="box-ul">
		...
		?>				<ul class="nav nav-stacked padding_<?php  echo $blogs['level'];?> <?php if(!$blogs['display']) echo 'blogs-widget-display-none'; ?>">
					<li><a href="<?php if($blogs['act']) echo $blogs['href']."#"; else echo $blogs['href']; ?>" class="sc_widget_blogs_a <?php if($blogs['act']) echo 'active'; if(!$blogs['act']) echo 'pass'; ?>">
						<?php if (isset($settings_widget['thumb_status']) && $settings_widget['thumb_status'] && $blogs['thumb']) { ?>
			            	<img src="<?php echo $blogs['thumb']; ?>" class="sc_widget_blogs_img" title="<?php echo $blogs['name']; ?>" alt="<?php echo $blogs['name']; ?>">
						<?php } ?>
