...
<div class="order__delivery">
                <h3><img src="img/order-del.svg" alt="">2. Способ доставки</h3>
                <p>Пожалуйста, выберете способ доставки</p>
...
<label class="order__radio" for="<?php echo $quote['code']; ?>">
...
<?php } ?>
<span></span> <?php echo !empty($quote['title']) ? $quote['title'] : ''; ?><?php echo !empty($quote['text']) ? ' - ' . $quote['text'] : ''; ?>
</label>

<?php if (!empty($quote['description']) && (!$display_for_selected || ($display_for_selected && $quote['code'] == $code))) { ?>
<div class="form-group"><label for="<?php echo $quote['code']; ?>"><?php echo $quote['description']; ?></label>
</div>
<?php } ?>
<?php if ($quote['code'] == $code && !empty($rows)) { ?><?php foreach ($rows as $row) { ?><?php echo $row ?><?php } ?>
                            <?php } ?>
                        <?php } ?>
                    <?php } else { ?>
