...
<form action="<?php echo $action; ?>" method="POST" onsubmit="return confirmSocnetauthForm();"
...
<?php /* start 0102 */ ?>
<?php if( $is_group && $groups ) { ?>
<tr>
	<td width=80><?php echo $entry_group; ?><?php if( $group_required ) echo "*"; ?></td>
	<td><select name="group">
	<?php foreach( $groups as $groupp) { ?>
	<option value="<?php echo $groupp['customer_group_id']; ?>"
	<?php if( $groupp['customer_group_id'] == $group ) { ?> selected <?php } ?>
	><?php echo $groupp['name']; ?></option>
	<?php } ?>
	</select></td>
</tr>
<?php } ?>
...
<?php if( $count_agrees && $inf ) { ?>
<tr> 
	<td colspan=2><?php echo $entry_agree; ?>
	
	<?php if( $is_agree ) {?>
	<div class="agree"><input type="checkbox" name="agree" value="1"
	checked
	>&nbsp;&nbsp;&nbsp;<a href="<?php echo $inf['href']; ?>" target=_blank><?php echo $inf['title']; ?></a></div>
	<?php if( $error_agree ) { ?><div class="err"><?php echo $error_agree; ?></div><?php } ?>
	<?php } ?>
	<?php if( $is_agree2 ) {?>
	<div class="agree"><input type="checkbox" name="agree2" value="1"
	checked
	>&nbsp;&nbsp;&nbsp;<a href="<?php echo $inf2['href']; ?>" target=_blank><?php echo $inf2['title']; ?></a></div>
	<?php if( $error_agree2 ) { ?><div class="err"><?php echo $error_agree2; ?></div><?php } ?>
	<?php } ?>
	<?php if( $is_agree3 ) {?>
	<div class="agree"><input type="checkbox" name="agree3" value="1"
	checked
	>&nbsp;&nbsp;&nbsp;<a href="<?php echo $inf3['href']; ?>" target=_blank><?php echo $inf3['title']; ?></a></div>
	<?php if( $error_agree3 ) { ?><div class="err"><?php echo $error_agree3; ?></div><?php } ?>
	<?php } ?>
...
<tr>
	<td width=80></td>
	<?php if( $is_noiframe ) { ?>
		<td><input type="button" onclick="confirmSocnetauthForm();" value="<?php echo $text_submit; ?>"></td>
	<?php } else { ?>
...
<?php /* start 1409 */ ?>
<?php if( $is_telephone && $telephone_mask ) { ?>
<script> 
  $('input[name="telephone"]').inputmask('<?php echo $telephone_mask; ?>');
</script> 
<?php } ?>
<?php /* end 1409 */ ?>