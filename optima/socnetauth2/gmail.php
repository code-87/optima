...
if( !empty($_GET['first']) )
{
	...
	setcookie("gm_state", $STATE, time()+3600, "/" );
	...
	$recordData = $SocAuth->getRecord( $_COOKIE['gm_state'] ) )
{
	...
	{	
		...
		$arr = array(
			...
			'lastname'  => isset( $arr['family_name'] ) ? $arr['family_name'] : '',
			'email'     => $arr['email'],
			'telephone'	=> ''
		);
		...
		if( $customer_id = $SocAuth->checkNew($data) )
		{
			if( $SocAuth->get_config_param('socnetauth2_dobortype') != 'every' )
			{
				...
				if( $IS_DEBUG ) exit( "END-1 ".$CURRENT_URI."<hr>Отладочные сообщения можно отключить во вкладке Gmail в настройках модуля");
			...
			else
			{
				if( $confirm_data = $SocAuth->isNeedConfirm( $data ) )
				{
					...
					if( $IS_DEBUG ) exit( "END-2 ".$CURRENT_URI."<hr>Отладочные сообщения можно отключить во вкладке Gmail в настройках модуля");
					header("Location: ".$CURRENT_URI ); 
				}
				else
				{
					...
					if( $IS_DEBUG ) exit( "END-3 ".$CURRENT_URI."<hr>Отладочные сообщения можно отключить во вкладке Gmail в настройках модуля");
					header("Location: ".$CURRENT_URI ); 
				}
			}
		}
		else
		{
			...
			if( !$SocAuth->get_config_param('socnetauth2_email_auth') || $SocAuth->get_config_param('socnetauth2_email_auth') == 'none' )
			{
				if( $confirm_data )
				{
					...
					if( $IS_DEBUG ) exit( "END-4 ".$CURRENT_URI."<hr>Отладочные сообщения можно отключить во вкладке Gmail в настройках модуля");
					header("Location: ".$CURRENT_URI ); 
				}
				else
				{
					...
					if( $IS_DEBUG ) exit( "END-5 ".$CURRENT_URI."<hr>Отладочные сообщения можно отключить во вкладке Gmail в настройках модуля");
					header("Location: ".$CURRENT_URI ); 
				}
			}
			elseif( $SocAuth->get_config_param('socnetauth2_email_auth') == 'confirm'  )
			{
				...
				if( $confirm_data )
				{
					...
					if( $IS_DEBUG ) exit( "END-6 ".$CURRENT_URI."<hr>Отладочные сообщения можно отключить во вкладке Gmail в настройках модуля");
					header("Location: ".$CURRENT_URI ); 
				}
				...
				elseif( !empty( $data['email'] ) && $SocAuth->checkByEmail($data, 0) )
				{
					...
					if( $IS_DEBUG ) exit( "END-7 ".$CURRENT_URI."<hr>Отладочные сообщения можно отключить во вкладке Gmail в настройках модуля");
					header("Location: ".$CURRENT_URI );
				}
					 ...
					 ( !empty( $data['email'] ) && !$SocAuth->checkByEmail($data, 0) ) )
				{
					...
					//echo "CID:".$customer_id."==========<hr>";
					//exit();
					
					$SocAuth->setSessionData('customer_id', $customer_id, 1);
					$SocAuth->setSessionCode( $customer_id ); 	
				
					if( $IS_DEBUG ) exit( "END-8 ".$CURRENT_URI."<hr>Отладочные сообщения можно отключить во вкладке Gmail в настройках модуля");
					header("Location: ".$CURRENT_URI ); 
				}
			}
			elseif( $SocAuth->get_config_param('socnetauth2_email_auth') == 'noconfirm'  )
			{
				...
				if( $confirm_data )
				{
					...
					if( $IS_DEBUG ) exit( "END-9 ".$CURRENT_URI."<hr>Отладочные сообщения можно отключить во вкладке Gmail в настройках модуля");
					header("Location: ".$CURRENT_URI ); 
				}
				// Получен E-mail 
				elseif( !empty( $data['email'] ) && $customer_id = $SocAuth->checkByEmail($data, 1) )
				{
					...
					if( $IS_DEBUG ) exit( "END-10 ".$CURRENT_URI."<hr>Отладочные сообщения можно отключить во вкладке Gmail в настройках модуля");
					header("Location: ".$CURRENT_URI );
				}
				//Получен e-mail и он уникальный 
				elseif( empty( $data['email'] ) ||
					 ( !empty( $data['email'] ) && !$SocAuth->checkByEmail($data, 0) ) )
				{
					...
					if( $IS_DEBUG ) exit( "END-11 ".$CURRENT_URI."<hr>Отладочные сообщения можно отключить во вкладке Gmail в настройках модуля");
					header("Location: ".$CURRENT_URI ); 
				}	
			}
		}
